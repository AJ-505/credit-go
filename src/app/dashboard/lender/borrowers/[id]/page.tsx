...
import {
  ...
  Panel,
  StatusPill,
} from "@/components/lender/lender-shell";
      ...
      <PageHeader
        ...
        action={
          <Link
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-bold"
          >
        ...
        <div className="space-y-6">
          ...
          <Panel className="p-5">
            ...
            <div className="mt-5 flex h-44 items-end gap-2 rounded-lg bg-slate-50 p-4">
              {[42, 54, 61, 58, 66, 70, 64, 74, 83, 78, 86, 92].map(
                (height, index) => (
                  <div
                    key={index}
                    className="flex-1 rounded-t bg-blue-800"
                    style={{ height: `${height}%` }}
                  />
                ),
              )}
        ...
        <Panel className="h-fit p-5">
          ...
          <div className="mt-4 space-y-3">
            {[
              "Paid",
              "Paid",
              borrower.health === "Alert" ? "Missed" : "Pending",
              "Pending",
            ].map((status, index) => (
              <div
                key={index}
          ...
          <div className="mt-5 rounded-lg bg-amber-50 p-4 text-sm font-semibold text-amber-900">
            Early warning: if this borrower's savings streak drops to 0 and the
            vault does not recover in 7 days, the account is escalated for
            review.
          </div>
        </Panel>
      </div>
